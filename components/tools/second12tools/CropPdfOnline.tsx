...
export default function CropPdfOnline() {
  ...
  const redrawCanvas = () => {
    ...
    img.onload = () => {
      ...
      if (cropArea) {
        // Apply zoom to crop area coordinates
        const zoomedCrop = {
          x: cropArea.x * zoom,
          y: cropArea.y * zoom,
        ...
        ctx.fillRect(0, 0, canvas.width, zoomedCrop.y);
        ctx.fillRect(0, zoomedCrop.y, zoomedCrop.x, zoomedCrop.height);
        ctx.fillRect(zoomedCrop.x + zoomedCrop.width, zoomedCrop.y, canvas.width - (zoomedCrop.x + zoomedCrop.width), zoomedCrop.height);
        ctx.fillRect(0, zoomedCrop.y + zoomedCrop.height, canvas.width, canvas.height - (zoomedCrop.y + zoomedCrop.height));

        // Draw crop rectangle
        ctx.strokeStyle = "#ff6b35";
        ctx.lineWidth = 3;
        ctx.strokeRect(zoomedCrop.x, zoomedCrop.y, zoomedCrop.width, zoomedCrop.height);
        ...
        // Corner handles
        ctx.fillRect(zoomedCrop.x - handleSize/2, zoomedCrop.y - handleSize/2, handleSize, handleSize);
        ctx.fillRect(zoomedCrop.x + zoomedCrop.width - handleSize/2, zoomedCrop.y - handleSize/2, handleSize, handleSize);
        ctx.fillRect(zoomedCrop.x - handleSize/2, zoomedCrop.y + zoomedCrop.height - handleSize/2, handleSize, handleSize);
        ctx.fillRect(zoomedCrop.x + zoomedCrop.width - handleSize/2, zoomedCrop.y + zoomedCrop.height - handleSize/2, handleSize, handleSize);

        // Edge handles
        ctx.fillRect(zoomedCrop.x + zoomedCrop.width/2 - handleSize/2, zoomedCrop.y - handleSize/2, handleSize, handleSize);
        ctx.fillRect(zoomedCrop.x + zoomedCrop.width/2 - handleSize/2, zoomedCrop.y + zoomedCrop.height - handleSize/2, handleSize, handleSize);
        ctx.fillRect(zoomedCrop.x - handleSize/2, zoomedCrop.y + zoomedCrop.height/2 - handleSize/2, handleSize, handleSize);
        ctx.fillRect(zoomedCrop.x + zoomedCrop.width - handleSize/2, zoomedCrop.y + zoomedCrop.height/2 - handleSize/2, handleSize, handleSize);
      }
    };
  };
  ...
  const getResizeHandle = (x: number, y: number): string => {
    ...
    const handleSize = 12;
    const tolerance = handleSize / zoom; // Adjust tolerance for zoom
  ...
  const handleZoomIn = () => {
    ...
    showToast(`Zoom: ${Math.round((zoom + 0.25) * 100)}%`);
  };
      ...
      <div className="w-full max-w-6xl relative z-10">
        ...
        <div className="text-center mb-8 sm:mb-12">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold mb-3 text-gray-900">
            Crop <span className="text-orange-500">PDF Online</span>
          </h1>
                ...
                <div className="bg-white rounded-xl p-4 border border-gray-200">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="px-3 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition-all min-w-[70px]"
                        title="Reset zoom"
                      >
                  ...
                  <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <p className="text-sm text-blue-800">
                      <strong>Tip:</strong> Use zoom controls to see details. Drag to create crop area. Use handles to resize. Click and drag inside to move.
                    </p>
                  </div>
                </div>
